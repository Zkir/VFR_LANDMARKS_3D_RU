...
class RoofSkillion(Roof):
    ...
    def make(self, osm):
        ...
        # Important: ensure that the first two vertices of each wall face will be always
        ...
        if self.noWalls:
            ...
            # definitely not (indices[index]) -- it's from roof
            bottomIndicies.append(indices[leftIndex])
            bottomIndicies.append(indices[rightIndex])
            while True:
                prevIndex = index
                ...
                bottomIndicies.append(indexOffset)
                indexOffset += 1
            ...
            #print("wallIndices",wallIndices)
            self.bottomIndices.append (tuple(bottomIndicies) )
        else:
            # vertices for the bottom part
            verts.extend(Vector((v.x, v.y, self.z1)) for v in polygon.verts)
            self.bottomIndices.append (tuple(indexOffset+i for i in range(len(list(polygon.verts))))  )
